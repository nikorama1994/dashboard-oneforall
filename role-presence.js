(function(){
  ...
  function forceMenuVisibility(context){
    const p=context?.profile||{};
    let role=String(p.role||'guest_internal').toLowerCase();
    ...
    function hideSystemLabel(){
      document.querySelectorAll('.sidebarSystemLabel').forEach(function(el){
        el.hidden=true;
        el.setAttribute('aria-hidden','true');
        el.style.setProperty('display','none','important');
      });
    ...
    // Reset dulu agar perpindahan role/session tidak meninggalkan state lama.
    ['sideStock','sideDatabase','sideBackup','sideSettings'].forEach(show);
    showSystemLabel();

    if(role==='qc_inspector'){
      // QC Inspector: hanya Backup Data yang tampak di SYSTEM.
      hide('sideDatabase');
      hide('sideSettings');
      show('sideBackup');
      showSystemLabel();

    }else if(role==='guest_internal'){
      // Guest Internal: viewer, SYSTEM hilang, Tembakau tetap terlihat.
      hide('sideDatabase');
      hide('sideBackup');
      hide('sideSettings');
      hideSystemLabel();
    ...
    }else if(role==='guest_external'){
      // Guest Eksternal: viewer, SYSTEM hilang, Tembakau juga hilang.
      hide('sideDatabase');
      hide('sideBackup');
      hide('sideSettings');
      hide('sideStock');
    ...
    // Admin: reset default di atas = semua terlihat.
  }
    ...
    ['sideDatabase','sideSettings'].forEach(function(id){
      ...
      if(el){
        ...
        el.title=id==='sideDatabase'
          ? 'Database tidak tersedia untuk QC Inspector'
          : 'Pengaturan tidak tersedia untuk QC Inspector';
      }
    });
  }
